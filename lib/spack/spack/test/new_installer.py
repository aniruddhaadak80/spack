...
class TestScheduleBuilds:
    """Unit tests for the module-level schedule_builds() free function."""
    ...
    def test_capacity_zero_returns_empty(self, temporary_store, mock_packages):
        """With capacity=0, nothing is scheduled and pending_builds is unchanged."""
        spec = self._make_spec("trivial-install-test-package")
        pending = [spec.dag_hash()]
        bg = _FakeBuildGraph([spec])
        jobserver = JobServer(num_jobs=2)
        try:
            ...
            assert not to_start
            assert not newly_installed
            assert len(pending) == 1  # still in queue
        finally:
            jobserver.close()
