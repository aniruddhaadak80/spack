...
def extend_flag_list(flag_list: List[str], new_flags: Sequence[str]):
...
# We have to take some care with how we serialize self.answers, because it contains
# specs that are not exactly concrete and also not exactly abstract. The result is
# cached after the solve and before post-processing, so not all specs are finalized and
# any hashes are not yet accurate.
...
class Result:
    ...
    def from_dict(obj: dict):
        ...
        result = Result(abstract_specs, obj["asp"])
...
class PyclingoDriver:
    ...
    def solve(
        ...
        if not result:
            ...
            for _, _, spec_dict in result.answers:
                post_process_fresh_solve(spec_dict, splices)
